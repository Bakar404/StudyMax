  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .insert([{
        ...
        task_description: taskData.task_description || taskData.taskDescription,
...
export const uploadTaskFile = async (file) => {
  try {
    ...
    const fileName = `${user.id}/tasks/${Date.now()}-${Math.random().toString(36).substring(7)}.${fileExt}`;
...
export const addTaskFile = async (taskId, file) => {
  try {
    ...
    const fileExt = file.name.split('.').pop();
    const fileName = `${user.id}/tasks/${taskId}/${Date.now()}-${Math.random().toString(36).substring(7)}.${fileExt}`;
