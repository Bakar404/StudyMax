...
export const SUPPORTED_FILE_TYPES = {
  ...
  // Presentations
  ppt: { mime: 'application/vnd.ms-powerpoint', ext: '.ppt', category: 'document' },
  pptx: { mime: 'application/vnd.openxmlformats-officedocument.presentationml.presentation', ext: '.pptx', category: 'document' },
  
  // Spreadsheets
  xls: { mime: 'application/vnd.ms-excel', ext: '.xls', category: 'document' },
  xlsx: { mime: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', ext: '.xlsx', category: 'document' },
  ...
  rar: { mime: 'application/x-rar-compressed', ext: '.rar', category: 'archive' },
};
...
export const isFileTypeSupported = (file) => {
  ...
  if (mimeType && !mimeType.startsWith(supportedType.category)) {
    ...
    if (mimeType !== supportedType.mime && !mimeType.includes(ext)) {
      console.warn(`MIME type mismatch for .${ext}: expected ${supportedType.mime}, got ${mimeType}`);
    }
  }
...
export const getAcceptString = () => {
  return Object.values(SUPPORTED_FILE_TYPES).map(type => type.ext).join(',');
};
...
export const getSupportedFormatsText = () => {
  const extensions = Object.keys(SUPPORTED_FILE_TYPES).map(ext => ext.toUpperCase());
