...
function ClassDetail({ classData, documents, onClose, onUpdate }) {
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      const uploadResult = await uploadDocument(file, classData.courseTitle || classData.course_title);
          ...
          <div className="detail-section">
            ...
            {(classData.courseDescription || classData.course_description) && (
              <div className="detail-item">
                ...
                <p>{classData.courseDescription || classData.course_description}</p>
              </div>
            )}
            {classData.days && classData.days.length > 0 && (
              <div className="detail-item">
                <label>Days:</label>
                <span>{Array.isArray(classData.days) ? classData.days.join(", ") : classData.days}</span>
              </div>
            )}
            {classData.time && (
              <div className="detail-item">
                ...
                {classDocuments.map((doc) => (
                  <div key={doc.id} className="attached-file">
                    <div className="file-info">
                      <div className="file-name">{doc.document_title || doc.documentTitle}</div>
                      <div className="file-meta">
                        {doc.document_type && <span className="file-type">{doc.document_type}</span>}
                        {(doc.file_size || doc.fileSize) && <span className="file-size">{formatFileSize(doc.file_size || doc.fileSize)}</span>}
                      </div>
                    </div>
